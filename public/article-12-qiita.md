...
## 次の記事: 13項目の実装ルールで AI の罠を構造的に防ぐ

「本番から import せよ」のようなプロンプト改善は効きますが、**人力で毎回思い出すのは限界** があります。SFAD の `impl` コマンドには、AI が破りがちな罠を 13 項目の実装ルールとして明文化しました:

1. Mass Assignment 防止 (Pydantic allow-list)
2. 認可チェック (Authorization Matrix 準拠)
3. N+1 クエリ防止 (selectinload/JOIN 必須)
4. SSR 安全性 (hydration mismatch 禁止)
5. **テストの本物性 ★本記事のテーマ**
6. 入力バリデーション境界
7. トランザクション境界の明示
8. エラーハンドリング (rethrow vs suppress)
9. リソースリーク防止 (context manager)
10. シークレット漏洩防止 (構造化ログ)
11. 冪等性 (リトライ安全)
12. レート制限考慮
13. 型変換の明示 (int / float / Decimal)

各項目を **静的解析ゲート + プロンプト制約 + レビューチェックリスト** の 3 段構えで強制します。次回の記事で全 13 項目を事例付きで解説します。

次回: **「13項目の実装ルール ― AIが破る罠リスト」** (5/21 公開予定)
